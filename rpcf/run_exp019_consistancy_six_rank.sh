...
BASE_RUN_ID="${BASE_RUN_ID:-exp019_seed43_fold0_full_20260615_1821}"
BASE_LOG_ROOT="${BASE_LOG_ROOT:-logs/exp019/${BASE_RUN_ID}}"
BASE_TAG="${BASE_RUN_ID//[^A-Za-z0-9_.-]/_}"

RUN_ID="${EXP019_CONSISTANCY_SIX_RANK_RUN_ID:-exp019_seed${SEED}_fold${FOLD}_consistancy_six_rank_$(date +%Y%m%d_%H%M%S)}"
...
if [[ "${DATASET}" != "thubenchmark" || "${MODEL}" != "eegnet" || "${EPS}" != "0.03" ]]; then
  echo "EXP-019 consistancy six-rank rerun currently supports thubenchmark/eegnet/eps0.03 only." >&2
  exit 1
fi
...
RPCF_SELECTIVE_CHECKPOINT="checkpoints/${DATASET}_${MODEL}_${PROTOCOL}_madry_eps${EPS}_${SEED}_fold${FOLD}_rpcf_selective_${BASE_TAG}_best.pth"
RPCF_ALL_CHECKPOINT="checkpoints/${DATASET}_${MODEL}_${PROTOCOL}_madry_eps${EPS}_${SEED}_fold${FOLD}_rpcf_all_layers_${BASE_TAG}_best.pth"
RPCF_UNIFORM_CHECKPOINT="checkpoints/${DATASET}_${MODEL}_${PROTOCOL}_madry_eps${EPS}_${SEED}_fold${FOLD}_rpcf_rank_weight_uniform_${BASE_TAG}_best.pth"
...
AT_PURIFY="purified_data/exp019/eval/${BASE_TAG}_madry_at_rank25-30.pth"
RPCF_SELECTIVE_PURIFY="purified_data/exp019/eval/${BASE_TAG}_rpcf_selective_rank25-30.pth"
RPCF_ALL_PURIFY="purified_data/exp019/eval/${BASE_TAG}_rpcf_all_layers_rank25-30.pth"
RPCF_UNIFORM_PURIFY="purified_data/exp019/eval/${BASE_TAG}_rpcf_rank_weight_uniform_rank25-30.pth"

SENSITIVITY_PATH="${BASE_LOG_ROOT}/sensitivity.json"
SELECTIVE_HISTORY="${BASE_LOG_ROOT}/finetune_selective"
ALL_HISTORY="${BASE_LOG_ROOT}/finetune_all_layers"
UNIFORM_HISTORY="${BASE_LOG_ROOT}/finetune_rank_weight_uniform"
...
mkdir -p \
  "${LOG_ROOT}" "${PAIR_DIR}" checkpoints ad_data/exp019 purified_data/exp019/eval
...
EXPERIMENT_ID=EXP-019-consistancy-six-rank
RUN_ID=${RUN_ID}
...
echo "[$(date -Is)] EXP-019 consistancy six-rank rerun finished: ${RUN_ID}"
