...
PAIR_DIR="purified_data/exp019/train_pair_consistancy"
PAIR25="${PAIR_DIR}/${DATASET}_${MODEL}_no_ea_fold${FOLD}_seed${SEED}_train_pair_consistancy_autoattack_eps${EPS}_PTR3d_8_2048_rank25_3d_interpolate_n${TRAIN_CACHE_SAMPLE_NUM}_tag${CONSISTANCY_TAG}.pth"
PAIR30="${PAIR_DIR}/${DATASET}_${MODEL}_no_ea_fold${FOLD}_seed${SEED}_train_pair_consistancy_autoattack_eps${EPS}_PTR3d_8_2048_rank30_3d_interpolate_n${TRAIN_CACHE_SAMPLE_NUM}_tag${CONSISTANCY_TAG}.pth"
...
CONSISTANCY_ATTACK="ad_data/exp019/${TAG}_consistancy_autoattack.pth"
...
CONSISTANCY_PURIFY="purified_data/exp019/eval/${TAG}_consistancy_rank25-30.pth"
...
if should_run 2; then
  ...
  for rank in 25 30; do
    if [[ "${rank}" == "25" ]]; then output="${PAIR25}"; else output="${PAIR30}"; fi
    if [[ "${SKIP_EXISTING}" == "1" && -f "${output}" ]]; then
      echo "[Stage 2] Reuse consistancy pair rank${rank}: ${output}"
      continue
    fi
...
if should_run 3; then
  ...
  if [[ "${SKIP_EXISTING}" == "1" && -f "${CONSISTANCY_CHECKPOINT}" ]]; then
    echo "[Stage 3] Reuse consistancy checkpoint: ${CONSISTANCY_CHECKPOINT}"
  else
