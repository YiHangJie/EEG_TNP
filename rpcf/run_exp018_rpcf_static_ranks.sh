...
RUN_ID="${EXP018_RPCF_STATIC_RUN_ID:-exp018_rpcf_static_ranks_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="${EXP018_RPCF_STATIC_LOG_ROOT:-logs/exp018/${RUN_ID}}"
...
RPCF_CHECKPOINT="checkpoints/${DATASET}_${MODEL}_${PROTOCOL}_madry_eps${EPS}_${SEED}_fold${FOLD}_rpcf_static_ranks_${TAG}_best.pth"
RPCF_ATTACK="ad_data/exp018/${TAG}_rpcf_static_ranks_autoattack.pth"
RPCF_PURIFY="purified_data/exp018/eval/${TAG}_rpcf_static_ranks_rank25-30.pth"
...
if should_run 1; then
  run_logged stage1_finetune_static_ranks \
    ...
    --static_rank_weights --history_prefix "${HISTORY_PREFIX}"
fi
...
if should_run 2; then
  ...
  run_logged stage2_attack \
    ...
    --dataset "${DATASET}" --model "${MODEL}" --fold "${FOLD}" --seed "${SEED}" \
    --checkpoint_path "${RPCF_CHECKPOINT}" --method_tag rpcf_static_ranks \
...
echo "[$(date -Is)] EXP-018 RPCF static-ranks finished: ${RUN_ID}"
