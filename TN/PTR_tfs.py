...
class PTR_tfs(BaseTNModel):
    def __init__(self, target, stage, max_rank=256, dtype='float32', loss_fn_str="L2", use_TTNF_sampling=False, payload=1, payload_position='first_core', regularization_type="TV", dimensions=2, regularization_weight = 0.0, noisy_target = None, device = 'cpu',  masked_avg_pooling = False, sigma_init=0, num_iterations=None, iterations_for_upsampling=None):
        ...
        self.S = target.shape[1]
        self.F = target.shape[2]
        self.T = target.shape[3]
        self.i = target.shape[0]
        self.stage = stage
        init_reso = self.S // (2 ** (stage-1))
        end_reso = self.T
        ...
        self.init_tn(self.S, self.F, self.T, self.stage)
    ...
    def init_tn(self, S, F, T, stage):
        """
        Initializes the tensor network (TN) for the model.
        """
        # Create the initial QTTNF
        self.tn = get_rr_template_qtr_eeg_tfs(S, F, T, stage, self.max_rank, dim=self.dimensions, sigma_init=self.sigma_init, device=self.device)      
        self.tn_tmp = get_rr_template_qtr_eeg_tfs(S, F, T, stage, self.max_rank, dim=self.dimensions, sigma_init=self.sigma_init, device=self.device)      
        # for i, core in enumerate(self.tn):
        #     print(f"core{i} shape:{core.shape}")

    def generate_targets_with_resos(self, img):
        '''
        img: the input, with shape (2, S, F, T)
        '''
        ...
        for factor in self.factors:
            low_res_img = torch.nn.functional.avg_pool3d(img.unsqueeze(0).clone(), kernel_size=(factor, factor, factor), stride=(factor, factor, factor))
    ...
    def custom_contract_qtt(self, output_reso):
        ...
        factor_shape = [2] + [2] * int(np.log2(self.S) + int(np.log2(self.F)) + np.log2(self.T))
        factor = output.reshape(factor_shape)
        factor_order = [0] + [i*3+1 for i in range(int(np.log2(self.S)))] + [i*3+2 for i in range(int(np.log2(self.F)))] + [i*3+3 for i in range(int(np.log2(self.T)))]
        factor = factor.permute(factor_order)
        output = factor.reshape(self.i, self.S, self.F, self.T)
        return output
    ...
    def tfs_TV(self, tensor, p=2):
        ...
        # Compute the squared differences in the diagonal direction
        diagonal_diff = torch.pow(abs(tensor[:, 1:, :] - tensor[:, :-1, :]), p)

        # Sum up the horizontal and vertical differences
        total_variation_loss = torch.sum(horizontal_diff) + torch.sum(vertical_diff) + torch.sum(diagonal_diff)
    ...
    def forward(self, reso):
        ...
        # loss = self.loss_fn(recon[:, :, :round(recon.shape[2]/2.5), :], self.targets[reso_index][:, :, :round(recon.shape[2]/2.5), :])
        loss = self.loss_fn(recon, self.targets[reso_index])

        # phase = torch.atan2(recon_[1, :, :, :], recon_[0, :, :, :])
        # magnititude = torch.sqrt(recon_[0, :, :, :]**2 + recon_[1, :, :, :]**2)
        # phase_gt = torch.atan2(self.targets[reso_index][1, :, :, :], self.targets[reso_index][0, :, :, :])
        # magnititude_gt = torch.sqrt(self.targets[reso_index][0, :, :, :]**2 + self.targets[reso_index][1, :, :, :]**2)
        # loss = self.loss_fn(magnititude, magnititude_gt) + self.loss_fn(phase, phase_gt)

        # loss += self.regularization_weight * self.tfs_TV(phase)
