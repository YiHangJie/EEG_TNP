...
- **核心设置：**
  - backbone：`eegnet`、`tsception`、`conformer`、`atcnet`，以及后续扩展的 `deepconvnet`、`tcnet`。
...
- **启动记录：**
  ...
  - 2026-07-09：新增 `deepconvnet`、`tcnet` 的前缀预算测试，只运行 `budget1/2/3/max`；分别指定物理 GPU2、GPU3，复用 EXP-024 对应的 Madry AT checkpoint、six-rank cache 和 sensitivity artifact。结果 Pending。
- **结果：** Pending
- **闭环检查：**
