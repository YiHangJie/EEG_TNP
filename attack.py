...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(
        f'./checkpoints/{args.dataset}_{args.model}_{protocol_tag}_{args.at_strategy}_eps{eps}_{args.seed}_fold{args.fold}_best.pth',
        map_location=device
    )
    ...
    torch.save((ad_data, labels), f'./ad_data/{args.dataset}_{args.model}_{protocol_tag}_{args.at_strategy}_{args.attack}_eps{args.eps}_{args.seed}_fold{args.fold}.pth')

    if args.at_strategy != 'clean':
        logging.info("Test model on 512 samples for adversarial training")
        test_data = []
        test_label = []
