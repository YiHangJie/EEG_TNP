...
from purify_train_pair_consistancy import (
    build_attack,
    load_classifier,
    load_train_batch,
    seed_everything,
...
from utils.experiment_artifacts import safe_token, short_protocol_tag
...
def parse_args():
    ...
    parser.add_argument('--output_tag', type=str, default='consistency2')
    parser.add_argument('--output_dir', type=str, default='./purified_data/train_pair_consistency2')
...
def build_output_path(args):
    """按 consistency2 实验关键信息生成稳定文件名，避免覆盖旧 consistancy 结果。"""
    ...
    file_name = (
        ...
        f'train_pair_consistency2_{args.attack}_eps{eps_tag}_{config_tag}_'
...
def main():
    ...
    config_tag = safe_token(os.path.splitext(os.path.basename(args.config))[0])
    log_path = (
        f'./log_purify/train_pair_consistency2_{args.dataset}_{args.model}_'
    ...
    logging.info('Purifying paired consistency2 training samples')
    logging.info(args)
    ...
    for batch_start in tqdm(range(0, len(selected_indices), args.batch_size), desc='purify train pair consistency2'):
        ...
        for offset, dataset_index in enumerate(batch_indices):
            ...
            logging.info(
                f'Purified consistency2 pair {local_index}, source_index: {dataset_index}, '
    ...
    logging.info(f'Saved paired consistency2 data: {output_path}')
    logging.info(
